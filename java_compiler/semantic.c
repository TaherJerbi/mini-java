...
int print_symtable() {
				...
				for(i = 0; i < st_index; i++){
								printf("%d\t%s\t%s\t%d\t%d\t%d\t%d\t%d\n",i, symbol_table[i].name, symbol_table[i].type, symbol_table[i].initialized, symbol_table[i].level, symbol_table[i].category, symbol_table[i].class_id, symbol_table[i].method_id);
